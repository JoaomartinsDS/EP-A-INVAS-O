...
def criador_lista(max, nome):
    list_k = []
    cont = 0
    while cont < max:
        l = int(input(f'{nome}{[cont]}: '))
        list_k.append(l)
        cont += 1
    return list_k
...
while n <= 0 or n >= 11:
    if n <= 0:
        ...
        n = int(input('n: '))
listm = criador_lista(m, 'm')
listn = criador_lista(n, 'n')
# soma dos m números
sm = 0
for i in listm:
    sm += i
...
if invdia == 0:
    invdia = 31
invmes = sn % 12
listmes = ['dezembro', 'janeiro', 'fevereiro', 'março', 'abril', 'maio', 'junho', 'julho', 'agosto', 'setembro',
           'outubro', 'novembro']
mes = listmes[invmes]
